...
class phy_map_impl : public tech_map_impl<Ntk, CutSize, CutData, NInputs, Configuration>
{
public:
  using parent = tech_map_impl<Ntk, CutSize, CutData, NInputs, Configuration>;
  ...
  explicit phy_map_impl(Ntk const& ntk, 
      ...
      : parent(ntk, library, ps, st),
        _congestion_map()  // Initialize the CongestionMap member
  {
  }
...
protected:
  ...
  void matchCongWire(node<Ntk> const& n, uint8_t phase) {
    ...
    auto index = parent::ntk.node_to_index(n);

    auto& node_data = parent::node_match[index];
    auto& cut_matches = parent::matches[index];
    supergate<NInputs> const* best_supergate = node_data.best_supergate[phase];

    /* recompute best match info */
    if (best_supergate != nullptr) {
      auto const& cut = parent::cuts.cuts(index)[node_data.best_cut[phase]];
      ...
      auto ctr = 0u;
      for (auto l : cut) {
        double arrival_pin = parent::node_match[l].arrival[(best_phase >> ctr) & 1] +
      ...
      if (cut.size() != 1) {
        ...
        gate_position = parent::match_position[index];
      }
      best_wirelength =
          parent::compute_match_wirelength(cut, gate_position, best_phase);
      best_total_wirelength =
          parent::compute_match_total_wirelength(cut, gate_position, best_phase);
    }

    /* foreach cut */
    for (auto& cut : parent::cuts.cuts(index)) {
      ...
      for (auto const& gate : *supergates[phase]) {
        ...
        double worst_wirelength =
            parent::compute_match_wirelength(*cut, gate_position, gate_polarity);
        double worst_total_wirelength =
            parent::compute_match_total_wirelength(*cut, gate_position, gate_polarity);

        auto ctr = 0u;
        for (auto l : *cut) {
          double arrival_pin =
              parent::node_match[l].arrival[(gate_polarity >> ctr) & 1] +
              gate.tdelay[ctr];
          worst_arrival = std::max(worst_arrival, arrival_pin);
          ++ctr;
        }

        if (worst_arrival > node_data.required[phase] + parent::epsilon) continue;

        if (!DO_TRADE) {
          if constexpr (!DO_POWER) {
            if constexpr (DO_TOTALWIRE) {
              if (worst_wirelength > node_data.wirelength[phase] + parent::epsilon)
                continue;
            }
          }
        }

        if (parent::compare_map_wirelength<DO_TOTALWIRE, DO_TRADE>(
    ...
    if (best_position){
      parent::match_position[index] = best_gate_position;
    } 
  }
...
binding_view<klut_network> phymap(
  ...
  map_stats st;
  detail::phy_map_impl<Ntk, CutSize, CutData, NInputs, Configuration, phyLS::RUDY<Ntk>> p{ntk, library, np, ps, st};
  auto res = p.run();
